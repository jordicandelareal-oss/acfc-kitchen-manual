-- MIGRATION 019: Dual Track Logistics (Option B) for Vegetarian Alternatives

-- 1. Function to get active vegetarians
CREATE OR REPLACE FUNCTION public.get_active_vegetarians() RETURNS integer AS $$
DECLARE
  v_count integer;
BEGIN
...
-- 2. Function to get vegetarian alternative
...
DECLARE
  ...
  v_alt_id uuid;
BEGIN
  SELECT name, is_vegetarian, category INTO v_name, v_is_veg, v_cat 
  FROM public.recipes WHERE id = main_recipe_id;
  
  -- If it's already vegetarian, return itself
  IF v_is_veg = true OR v_cat ILIKE 'Vegetariano' THEN
    RETURN main_recipe_id;
  END IF;

  -- Attempt 1: Exact name + (Vegetarian) or (vegetarian)
  SELECT id INTO v_alt_id FROM public.recipes 
  WHERE (name ILIKE v_name || ' (Vegetarian)%' OR name ILIKE v_name || ' (vegetarian)%')
    AND (is_vegetarian = true OR category ILIKE 'Vegetariano')
  LIMIT 1;

  IF v_alt_id IS NOT NULL THEN RETURN v_alt_id; END IF;

  -- Attempt 2: Just the first vegetarian recipe (fallback)
  SELECT id INTO v_alt_id FROM public.recipes 
...
-- 3. Replace generar_lista_compras_optimizada
